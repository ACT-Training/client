...
>
    ...
    <flux:button
        as="div"
        class="ms-2 -me-2 cursor-pointer [[data-flux-select-button]:has([data-flux-select-placeholder])_&]:hidden [[data-flux-select][disabled]:has([data-selected])_&]:hidden"
        variant="subtle"
        :size="$size === 'sm' ? 'xs' : 'sm'"
        square
        tabindex="-1"
        ...
        x-on:click.prevent.stop="
            let select = $el.closest('ui-select')
            select.value = select.hasAttribute('multiple') ? [] : null
    ...
    <flux:icon.chevron-down
        variant="mini"
        class="ms-2 -me-1 text-zinc-300 dark:text-white/60 [[data-flux-select-button]:hover_&]:text-zinc-800 dark:[[data-flux-select-button]:hover_&]:text-white [[disabled]_&]:text-zinc-200! dark:[[disabled]_&]:text-white/40!"
    />
</button>
