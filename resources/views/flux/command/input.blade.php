...
<div class="relative" data-flux-command-input>
    <div
        class="absolute start-0 top-0 bottom-0 flex items-center justify-center ps-3.5 text-xs text-zinc-400 [&:has(+input:focus)]:text-zinc-800 dark:[&:has(+input:focus)]:text-zinc-400"
    >
    ...
    <div class="absolute end-0 top-0 bottom-0 flex items-center justify-center pe-2">
        <ui-close>
    ...
    <div
        class="absolute end-0 top-0 bottom-0 flex items-center justify-center pe-2 [[data-flux-command-input]:has(input:placeholder-shown)_&]:hidden"
    >
        <flux:button
            square
            variant="subtle"
            ...
            x-on:click="
                $el.closest('[data-flux-command-input]').querySelector('input').value = ''
                $el.closest('[data-flux-command-input]')
                    .querySelector('input')
                    .dispatchEvent(new Event('input', { bubbles: false }))
                $el.closest('[data-flux-command-input]').querySelector('input').focus()
            "
        >
            <flux:icon.x-mark variant="micro" />
        </flux:button>
